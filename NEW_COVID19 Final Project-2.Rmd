...
The COVID-19 data used in this analysis originates from the Johns Hopkins University Center for Systems Science and Engineering (CSSE) time series repository, publicly available on GitHub. This comprehensive dataset tracks confirmed cases and deaths globally, with U.S. data reported at the county level while most other countries provide national aggregates. The time series tables are updated daily and represent cumulative counts, with U.S. records including geographic coordinates and population denominators for rate calculations. For Wisconsin-specific analysis, we utilized the U.S. county-level files (*time_series_covid19_confirmed_US.csv and time_series_covid19_deaths_US.csv*), which contain daily snapshots of pandemic progression from January 2020 through April 2022. The data collection methodology follows standardized case definitions from the World Health Organization, though reporting practices may vary slightly by jurisdiction. Each observation represents aggregated totals for a given county-date combination, enabling both temporal and geographic analysis of pandemic trends while maintaining individual privacy through aggregation. The dataset's structured format and daily updates make it particularly valuable for longitudinal studies of disease spread and outcomes.
